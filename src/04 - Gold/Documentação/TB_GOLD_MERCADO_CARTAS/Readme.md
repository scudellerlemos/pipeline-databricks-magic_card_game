# TB_GOLD_MERCADO_CARTAS

## 1. Nome da Tabela e Camada
- **Tabela:** TB_GOLD_MERCADO_CARTAS
- **Camada:** Gold
...
- **Usadas (4 de 6):** `TB_FATO_CARTAS` (driver), `TB_FATO_PRECOS_CARTAS` (INNER JOIN por `NME_CARTA`), `TB_DIM_COLECOES` (LEFT JOIN por `COD_COLECAO`), `TB_FATO_ESCLARECIMENTOS_CARTAS` (agregada por `ID_ORACLE`, LEFT JOIN).
- **Não usadas (2 de 6), desvio documentado:** `TB_DOM_SIMBOLOS` (tabela de referência de símbolos, sem chave de junção pro grão desta Gold) e `TB_MOV_MIGRACOES_CARTAS` (log de migração de id já resolvido dentro da própria Silver, sem grão compatível). Ver docstring de `TB_GOLD_MERCADO_CARTAS.py` para o raciocínio completo.
